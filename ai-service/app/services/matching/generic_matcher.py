...
class GenericMatchingEngine:
    ...
    def _calculate_skill_years(
            ...
            if contains_alias(exp_text):
                ...
                if start_d:
                    try:
                        ...
                        months = max(
                            1.0,
                            (dt_end.year - dt_start.year) * 12
                            + (dt_end.month - dt_start.month),
                        )
                        total_months += months
                    except Exception:
                        total_months += 6.0
        ...
        if total_months == 0:
            for cs in getattr(cand_profile, "skills", []) or []:
                ...
                if any(
                    alias == cs_name or alias in cs_name for alias in target_aliases
                ):
            ...
            if has_direct_kw:
                ...
                if score_val > best_score:
                    best_score, best_text, best_source = (
                        score_val,
                        exp_text,
                        f"Kinh nghiệm: {exp.position_title}",
                    )
                    is_explicit = True
    ...
    def _eval_projects(
        ...
        if not projects:
            return {"score": 0.2, "project_count": 0}
    ...
    def _match_certificates(
        ...
        if not cand_certs:
            return {
                "score": 0.2,
                "matched": [],
                "missing": [req.certificate_name for req in req_certs],
            }
        ...
        for req in req_certs:
            ...
            for c_name in cand_cert_names:
                if (
                    req.certificate_name.lower() in c_name.lower()
                    or c_name.lower() in req.certificate_name.lower()
                ):
                    best_sim = 1.0
                else:
                    best_sim = max(
        ...
        return {"score": final_score, "matched": matched, "missing": missing}
