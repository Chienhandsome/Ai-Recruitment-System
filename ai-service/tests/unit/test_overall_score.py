...
# pyrefly: ignore [missing-import]
from app.schemas.matching import EvaluationRequest, JobWeightsConfig
...
@pytest.mark.unit
def test_overall_weighted_score_formula(mock_semantic_matcher):
    """
    ...
    total_w = w.skills + w.experience + w.education + w.other

    expected_overall = (
        w.skills * response.skills_score
        + w.experience * response.experience_score
        + w.education * response.education_score
        + w.other * response.other_score
    ) / total_w

    if response.mandatory_score_cap is not None:
        expected_overall = min(expected_overall, response.mandatory_score_cap)
...
def test_custom_weights_impact(mock_semantic_matcher):
    ...
    payload_edu = make_evaluation_payload(
        weights={"skills": 10.0, "experience": 10.0, "education": 70.0, "other": 10.0}
    )
