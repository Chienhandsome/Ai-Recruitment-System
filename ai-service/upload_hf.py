...
def main():
    ...
    repo_id = input(
        "Vui lòng nhập Repo ID của bạn "
        "(Ví dụ: tathai/ai-recruitment-embedder): "
    ).strip()
    ...
    model_dir = os.path.abspath(
        os.path.join(
            os.path.dirname(__file__), "app", "models", "fine_tuned_embedder"
        )
    )
    ...
    print(
        "Đang tải lên (quá trình này có thể mất vài phút tùy vào mạng của bạn "
        "vì file nặng >500MB)..."
    )
    ...
    try:
        ...
        api.upload_folder(
            ...
            repo_id=repo_id,
            repo_type="model",
            commit_message="Upload fine-tuned Context-Aware embedding model V3",
        )
