...
async function main() {
    ...
    const jdSkillsData = [
        { name: 'Quản trị Paid Media E-commerce (Meta Ads & Google PMax)', norm: 'paid media ecommerce meta google pmax', man: true, lvl: client_1.ProficiencyLevel.EXPERT },
        { name: 'Tối ưu Phễu Chuyển đổi & Đơn vị Kinh tế E-com (ROAS, CAC, MER)', norm: 'unit economics ecommerce roas cac mer', man: true, lvl: client_1.ProficiencyLevel.ADVANCED },
        { name: 'Hạ tầng Tracking Chuyên sâu (GTM Server-Side, GA4, Meta CAPI)', norm: 'tracking gtm server side ga4 meta capi', man: true, lvl: client_1.ProficiencyLevel.ADVANCED },
        { name: 'Tối ưu Nền tảng D2C & Sàn TMĐT (Shopify, TikTok Shop)', norm: 'd2c shopify tiktok shop ecommerce', man: false, lvl: client_1.ProficiencyLevel.INTERMEDIATE },
        { name: 'A/B Testing & Tối ưu Tỷ lệ Chuyển đổi (CRO)', norm: 'cro ab testing conversion rate optimization', man: false, lvl: client_1.ProficiencyLevel.INTERMEDIATE },
    ];
    const jdSkills = [];
    ...
    for (const js of jdSkills) {
        await prisma.jobSkill.create({
            data: {
                jobId: jobPosting.id,
                skillId: js.skill.id,
                requirementType: js.man ? client_1.SkillRequirementType.MANDATORY : client_1.SkillRequirementType.PREFERRED,
        ...
        const evalPayload = {
            ...
            candidate_profile: {
                profile: {
                    ...
                    professional_summary: c.profile.summary ?? c.profile.professional_summary,
                },
    ...
    for (const r of benchmarkResults) {
        ...
        r.gaps.forEach((g) => console.log(`   - ${g}`));
    }
    console.log('--------------------------------------------------------------------------------\n');
...
//# sourceMappingURL=run_adversarial_benchmark.js.map