...
async function main() {
    ...
    if (!recruiterProfile)
        throw new Error('RecruiterProfile not found');
    ...
    await prisma.jobSkill.createMany({
        data: [
            { jobId: jobPosting.id, skillId: createdSkills[0].id, requirementType: client_1.SkillRequirementType.MANDATORY, minimumProficiency: client_1.ProficiencyLevel.EXPERT },
            { jobId: jobPosting.id, skillId: createdSkills[1].id, requirementType: client_1.SkillRequirementType.MANDATORY, minimumProficiency: client_1.ProficiencyLevel.ADVANCED },
            { jobId: jobPosting.id, skillId: createdSkills[2].id, requirementType: client_1.SkillRequirementType.MANDATORY, minimumProficiency: client_1.ProficiencyLevel.ADVANCED },
            { jobId: jobPosting.id, skillId: createdSkills[3].id, requirementType: client_1.SkillRequirementType.PREFERRED, minimumProficiency: client_1.ProficiencyLevel.INTERMEDIATE },
            { jobId: jobPosting.id, skillId: createdSkills[4].id, requirementType: client_1.SkillRequirementType.PREFERRED, minimumProficiency: client_1.ProficiencyLevel.BEGINNER },
        ],
    });
    ...
    for (const cs of candSkillsData) {
        ...
        await prisma.candidateSkill.create({
            data: {
                ...
                skillId: sk.id,
                proficiencyLevel: client_1.ProficiencyLevel.ADVANCED,
            },
        });
    }
    ...
    result.matched_skills.forEach((m) => console.log(`     [✓] ${m.name} (Bắt buộc: ${m.isMandatory})`));
    if (result.missing_skills && result.missing_skills.length > 0) {
        console.log('\n  • Missing Skills (Kỹ năng thiếu hụt):');
        result.missing_skills.forEach((m) => console.log(`     [✗] ${m.name} (Bắt buộc: ${m.isMandatory})`));
    }
    console.log('\n  • Điểm mạnh (Strengths):');
    result.strengths.forEach((s) => console.log(`     ⭐ ${s}`));
    if (result.gaps && result.gaps.length > 0) {
        console.log('\n  • Điểm thiếu sót (Gaps):');
        result.gaps.forEach((g) => console.log(`     ⚠️ ${g}`));
    }
    console.log(`\n  • Tóm tắt đánh giá: \n     ${result.summary}`);
    console.log('================================================================================\n');
...
//# sourceMappingURL=setup_test_scenario.js.map