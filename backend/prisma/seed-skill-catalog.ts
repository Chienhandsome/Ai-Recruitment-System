...
const LEGACY_COMPOSITE_SKILL_MIGRATIONS: Readonly<
...
export async function seedSkillCatalog(
  prisma: PrismaClient,
  catalog: readonly SkillCatalogEntry[] = SKILL_CATALOG,
  ...
  const existingSkillByNormalizedName = new Map(
    existingSkills.map((skill) => [skill.normalizedName, skill]),
  );

  const newSkills = catalog.filter(
    (skill) => !existingSkillByNormalizedName.has(skill.normalizedName),
  );
  ...
  const skillUpdates = catalog.flatMap((skill) => {
    ...
    return [
      prisma.skill.update({
        ...
        data: { name: skill.name, categoryId, type, status: SkillStatus.ACTIVE },
      }),
    ];
  });
  ...
  const skillRecords = new Map<string, Skill>(
    seededSkills.map((skill) => [skill.normalizedName, skill]),
  );

  const legacyMigrationResult = await migrateLegacyCompositeSkills(
    prisma,
    skillRecords,
  );

  // Older seed scripts created aliases such as "React" for a composite
  // "React.js & Next.js" row. Such aliases compete with the canonical Skill
  // name during CV hydration, so remove only those redundant collisions.
  const canonicalSkillByLabel = new Map(
    [...skillRecords.values()].map((skill) => [
  ...
  const aliasGroups = new Map<string, typeof existingAliases>();
  for (const alias of existingAliases) {
    const key = normalizeLabel(alias.aliasName);
    aliasGroups.set(key, [...(aliasGroups.get(key) ?? []), alias]);
  }

  const aliasIdsToDelete = new Set<string>();
  for (const alias of existingAliases) {
  ...
  const aliasesToCreate: typeof desiredAliases = [];
  const aliasesToUpdate: Prisma.PrismaPromise<unknown>[] = [];
  ...
  for (const desiredAlias of desiredAliases) {
    ...
    const keeper =
      existingGroup.find((alias) => alias.aliasName === desiredAlias.aliasName) ??
      existingGroup[0];
    for (const duplicate of existingGroup) {
      if (duplicate.id !== keeper.id) aliasIdsToDelete.add(duplicate.id);
    }
    if (
      keeper.aliasName !== desiredAlias.aliasName ||
      keeper.skillId !== desiredAlias.skillId
    ) {
      aliasesToUpdate.push(
        prisma.skillAlias.update({
          where: { id: keeper.id },
          data: desiredAlias,
        }),
      );
...
function toPrismaSkillType(type: SkillCatalogEntry['type']): SkillType {
  return type === 'SOFT' ? SkillType.SOFT : SkillType.HARD;
}

function requiredCategoryId(
...
async function migrateLegacyCompositeSkills(
  prisma: PrismaClient,
  canonicalSkills: ReadonlyMap<string, Skill>,
): Promise<
  Pick<
    SkillCatalogSeedResult,
    | 'deprecatedLegacySkills'
    | 'migratedCandidateSkillLinks'
    | 'migratedJobSkillLinks'
  >
> {
  const legacySkills = await prisma.skill.findMany({
    where: {
      ...
      OR: [
        { status: { not: SkillStatus.INACTIVE } },
  ...
  const candidateLinks: Prisma.CandidateSkillCreateManyInput[] = [];
  const jobLinks: Prisma.JobSkillCreateManyInput[] = [];

  for (const legacySkill of legacySkills) {
    const targetKeys =
      LEGACY_COMPOSITE_SKILL_MIGRATIONS[legacySkill.normalizedName];
    const targetSkillIds = targetKeys.map((targetKey) => {
      const target = canonicalSkills.get(targetKey);
      if (!target) {
        throw new Error(
          `Legacy skill migration target "${targetKey}" is not in the canonical catalog.`,
        );
      }
      return target.id;
    });
  ...
  await prisma.$transaction([
    ...
    prisma.skill.updateMany({
      ...
      data: { status: SkillStatus.INACTIVE },
    }),
  ]);

  return {
    deprecatedLegacySkills: legacySkills.length,
    migratedCandidateSkillLinks: legacySkills.reduce(
      (sum, skill) => sum + skill.candidateSkills.length,
      0,
    ),
    migratedJobSkillLinks: legacySkills.reduce(
      (sum, skill) => sum + skill.jobSkills.length,
      0,
    ),
  };
}
