...
async function main() {
  ...
  if (!adminUserId) {
    throw new Error(
      'ADMIN_USER_ID is required. Create the user in Supabase Auth first.',
    );
  }

  if (!supabaseUrl || !supabaseSecretKey) {
    throw new Error(
      'SUPABASE_URL and SUPABASE_SECRET_KEY are required to verify the Auth user.',
    );
  }
  ...
  if (error || !data.user?.email) {
    throw new Error(
      `Cannot load Supabase Auth user ${adminUserId}: ${error?.message ?? 'email is missing'}`,
    );
  }
